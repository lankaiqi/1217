    ...
    object SessionAggrStatAccumulator extends AccumulatorParam[String] {
      ...
      def addInPlace(v1: String, v2: String): String = {
        ...
        if (v1 == "") {
          ...
          // 给连接串中的v2设置新的累加后的值
          val str = StringUtils.setFieldInConcatString(v1, "\\|", v2, newValue)
          str
        }
      }
    }
      //创建spark上下文
      val conf = new SparkConf().setAppName("SessionAggrStatAccumulatorTest").setMaster("local[1]");
      ...
      var arr = Array(Constants.TIME_PERIOD_1s_3s,Constants.TIME_PERIOD_4s_6s)
      var rdd = sc.parallelize(arr,1)
      rdd.foreach(aa=>{
        println("+++++++++++"+aa)
        sessionAggrstatAccumulator.add(aa)
        println("+++++++++++"+aa)
        }
      )
      println(sessionAggrstatAccumulator)
  }
}
